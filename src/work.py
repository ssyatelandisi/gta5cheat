...
class WorkSuspendProcess(QObject):
...
class WorkLuckyWheel(QObject):
    ...
    def slotWork(self):
        while True:
            self.gtav.set_luckyWheel(18)
            QThread.msleep(100)


class WorkStatHax(QObject):
...
class WorkCheckBoxWatch(QObject):
    ...
    def slotWork(self):
        while True:
            ...
            if code == 0x1131:  # LCTRL+1
                ...
                self.gtav.change_session(self.ui.sessions[self.ui.comboBox_session.currentIndex()][1])
                self.signalDuu.emit()
                QThread.msleep(150)
            elif code == 0x11C0:  # LCTRL+~
                self.gtav.spawn_vehicle(self.gtav.joaat(self.ui.vehicles[self.ui.comboBox_vehicles.currentIndex()][1]))
                self.signalDuu.emit()
                QThread.msleep(150)
            elif code == 0x6F:  # /
