...
class WindowApp(QWidget):
    ...
    signalWorkSuspend = pyqtSignal()
    signalWorkLuckyWheel = pyqtSignal()
    signalWorkStatHax = pyqtSignal()
    signalWorkCheckBoxWatch = pyqtSignal()
    ...
    def __init__(self, gtav: Gtav):
        ...
        self.threadSuspend = QThread()  # 挂起游戏子线程
        self.threadLuckyWheel = QThread()  # 幸运转盘子线程
        self.threadStatHax = QThread()  # stat输入子线程
        self.threadCheckBoxWatch = QThread()  # 复选框监听子线程
        self.threadHotkey = QThread()  # 热键监听子线程

        self.workSuspendProcess = WorkSuspendProcess(None, self.gtav)
        self.workLuckyWheel = WorkLuckyWheel(None, self.gtav)
        self.workCheckBoxWatch = WorkCheckBoxWatch(None, self.ui, self.model, self.gtav)
        self.workStatHax = WorkStatHax(None, self.ui, self.gtav)
        self.workHotkey = WorkHotkey(None, self.ui, self.gtav)

        self.threadSuspend.start()
        self.threadStatHax.start()
        self.threadCheckBoxWatch.start()
        self.threadHotkey.start()

        self.workSuspendProcess.moveToThread(self.threadSuspend)
        self.workLuckyWheel.moveToThread(self.threadLuckyWheel)
        self.workStatHax.moveToThread(self.threadStatHax)
        self.workCheckBoxWatch.moveToThread(self.threadCheckBoxWatch)
        self.workHotkey.moveToThread(self.threadHotkey)
        ...
        self.signalWorkLuckyWheel.connect(self.workLuckyWheel.slotWork)
        self.signalWorkStatHax.connect(self.workStatHax.slotWork)
        ...
        self.signalWorkHotkey.connect(self.workHotkey.slotWork)
        self.signalWorkHotkey.emit()
        self.signalWorkCheckBoxWatch.connect(self.workCheckBoxWatch.slotWork)
        self.signalWorkCheckBoxWatch.emit()
    ...
    def slotCheckBox_stateChanged(self, component: QCheckBox):
        ...
        if component.objectName() == "checkBox_luckyWheel":
            if self.ui.checkBox_luckyWheel.isChecked():
                if not self.threadLuckyWheel.isRunning():
                    self.threadLuckyWheel.start()
                    self.signalWorkLuckyWheel.emit()
            else:
                if self.threadLuckyWheel.isRunning():
                    self.threadLuckyWheel.terminate()
    ...
    def slotComboBox_changed(self, component: QComboBox):
        ...
        if component.objectName() == "comboBox_weather":
            ...
            self.task = Task(
                ...
                int(self.vehicleAbilities[component.currentIndex()][1]),
            )
            self.task.start()
    ...
    def change_vehicleAbility(self):
        """修改载特殊能力"""
        self.gtav.set_vehicleAbility(
            self.vehicleAbilities[self.ui.comboBox_vehicleAbilities.currentIndex()][1]
        )
    ...
    def change_weather(self):
        """修改天气"""
        self.gtav.set_weather(self.weather[self.ui.comboBox_weather.currentIndex()][1])
